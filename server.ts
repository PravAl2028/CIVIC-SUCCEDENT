...
async function startServer() {
  ...
  app.use((req, res, next) => {
    ...
    res.writeHead = function (statusCode, ...args) {
      ...
      if (cacheControl && typeof cacheControl === "string") {
        ...
        if (req.url.startsWith("/api")) res.setHeader("Cache-Control", "no-cache, no-store");
  ...
  // Scanner Agent — client sends image + coordinates + existing cases for dedup
  app.post("/api/agents/scanner", async (req, res) => {
    try {
      ...
      // Geo-deduplication using cases provided by client
  ...
  // Dispatcher Agent — client sends case data + previous letters context
  ...
  // Resolver Agent — client sends before/after images
  ...
  app.listen(port, "0.0.0.0", () => {
    console.log(`Nagarika Server running on http://0.0.0.0:${port}`);
  });
}
